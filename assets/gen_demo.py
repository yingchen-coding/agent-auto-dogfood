...
TITLE = 'agent-auto-dogfood — turn agent chat logs into a fix list'
LINES = [
    ('$ agent-auto-dogfood analyze ~/agent/transcripts', 'cmd'),
    ('  mined 741 messages  →  27 dissatisfaction signals', 'ok'),
    ('', 'blank'),
    ('  #1  repeated-failure  "still not fixed" ×6   (evidence: 6 sessions)', 'err'),
    ('  #2  unresolved        tool errors ×4', 'dim'),
    ('  #3  low evidence       claims without a check ×3', 'dim'),
    ('', 'blank'),
    ('  → prioritized, evidence-backed action items written', 'ok'),
]
